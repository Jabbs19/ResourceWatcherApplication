...
#Kwargs in future for namespaces, etc.
def create_api_client(apiVersion, authClient):

    api = eval('client.' + apiVersion + '(authClient)')
    
    return api

def create_apiWithResource_client(api,resourceString):

    apiWithResource = eval('api'+'.'+resourceString)

    return apiWithResource
